...
public:
  ...
  void state_clear(unsigned mask) {
    MDSCacheObject::state_clear(mask);
    if (!is_auth()) {
      loner_cap = -1;
    }
  }
  void state_set(unsigned mask) {
    MDSCacheObject::state_set(mask);
    if (!is_auth()) {
      loner_cap = -1;
    }
  }
  void state_reset(unsigned s) {
    MDSCacheObject::state_reset(s);
    if (!is_auth()) {
      loner_cap = -1;
    }
  }
  ...
  int get_client_cap_pending(client_t client) const {
    if (client_caps.count(client)) {
      return client_caps.find(client)->second->pending();
    } else {
      return 0;
    }
  }
